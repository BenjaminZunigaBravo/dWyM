...
class SimpleRoulette {
    ...
    updateBetHistoryTable() {
        ...
        displayResults.forEach(result => {
            ...
            row.innerHTML = `
                ...
                <td>$${result.amount}</td>
                <td>${result.result}</td>
                <td class="${variationClass}">${variationSign}${result.variation}</td>
            `;
    ...
    // =============================================          return window.rouletteInstance;
        ...
        this.timer = null;
        this.recentNumbers = [];
        this.betHistory = []; // Historial de apuestas
    ...
    init() {
        ...
        this.startTimer();
        this.updateBalance();
        this.updateRecentNumbers(); // Inicializar números recientes
    }
    ...
    placeBet(element, betType) {
        ...
        if (this.currentBets[betKey]) {
            this.showMessage(`Ya apostaste $${this.currentBets[betKey]} en ${betType}`, 'info');
            return;
        }
        ...
        element.innerHTML += `<div style="color: gold; font-weight: bold;">$${betAmount}</div>`;
        
        this.showMessage(`Apostaste $${betAmount} en ${betType}`, 'success');
    }
    ...
    processResult(winningNumber) {
        ...
        Object.keys(this.currentBets).forEach(betKey => {
            ...
            if (winAmount > 0) {
                ...
                this.showMessage(`¡Ganaste $${winAmount} con ${betType}!`, 'success');
            }
        });
